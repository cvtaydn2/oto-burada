          ...
          <div className="max-w-3xl text-left space-y-6 sm:space-y-8 lg:max-w-none lg:pl-2 xl:pl-4">
            ...
            <div className="space-y-3 sm:space-y-4">
              <h1 className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl xl:text-6xl font-bold text-foreground tracking-tight leading-[1.1]">
                Arabanı{" "}
                <span className="text-primary underline decoration-primary/30 underline-offset-4 sm:underline-offset-8">
                  Kolayca
                </span>{" "}
                Sat. <br />
              ...
              <p className="text-sm sm:text-base md:text-lg text-muted-foreground max-w-xl font-medium leading-relaxed">
                Türkiye&apos;nin en güvenilir, şeffaf ve sade otomobil pazarı. Gereksiz detaylardan
                uzak, sadece sonuca odaklı.
              </p>
            </div>
            ...
            >
              <div className="flex-1 flex flex-col sm:flex-row gap-1 sm:gap-2 p-1 sm:p-2">
                <div className="flex-1 relative">
                  ...
                  <CarFront
                    size={16}
                    className="absolute left-3 sm:left-6 top-1/2 -translate-y-1/2 text-muted-foreground"
                  />
                  <input
                    id="hero-query"
                    type="text"
                ...
                <div className="w-full md:w-56 relative">
                  ...
                  <MapPin
                    size={16}
                    className="absolute left-3 sm:left-6 top-1/2 -translate-y-1/2 text-muted-foreground pointer-events-none"
                  />
                  <select
                    id="hero-city"
                    name="city"
                  ...
                  >
                    ...
                    {cities.map((city) => (
                      <option key={city} value={city} className="bg-card">
                        {city}
                      </option>
                    ))}
                  </select>
            ...
            <div className="flex items-center gap-4 sm:gap-6 md:gap-8 pt-4 sm:pt-6">
              <div className="flex flex-col">
                <span className="text-xl sm:text-2xl font-bold text-foreground leading-none tracking-tighter">
                  %100
                </span>
                <span className="text-[11px] text-muted-foreground font-bold uppercase tracking-widest mt-0.5 sm:mt-1">
                  Ücretsiz İlan
                </span>
              </div>
