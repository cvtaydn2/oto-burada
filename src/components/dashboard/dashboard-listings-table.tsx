  ...
  if (listings.length === 0) {
    return (
      <div className="py-20 text-center bg-card rounded-2xl border border-dashed border-border">
        ...
        <p className="mt-1 text-sm font-medium text-muted-foreground">Henüz ilan yayınlamamışsınız.</p>
        <Button className="mt-8 rounded-xl h-11 px-10 text-[10px] font-bold uppercase tracking-widest shadow-sm shadow-primary/20" asChild>
  ...
  return (
    <div className="rounded-xl border border-border bg-card p-6 lg:p-8 shadow-sm">
      <div className="mb-8 flex items-center justify-between">
        ...
        <Link href="/dashboard/listings" className="flex items-center gap-2 rounded-xl bg-muted h-10 px-5 text-[10px] font-bold uppercase tracking-widest text-foreground transition-all hover:bg-muted/80">
          TÜMÜ
              ...
              <tr key={listing.id} className="group hover:bg-muted/20 transition-colors">
                ...
                <td className="py-4">
                  <div className={cn(
                    ...
                    listing.status === "approved" ? "bg-emerald-50 text-emerald-600 border border-emerald-100" :
