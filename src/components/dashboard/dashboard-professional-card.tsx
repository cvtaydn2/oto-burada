...
export function DashboardProfessionalCard({ profile }: DashboardProfessionalCardProps) {
          ...
          <div className="space-y-4">
            ...
            <div>
              <h2 className="text-2xl font-bold tracking-tight">
                Kendi Galerini Aç{" "}
                <span className="text-amber-400/40 font-medium ml-2">Premium</span>
              </h2>
              <p className="text-sm font-medium text-slate-400 max-w-lg mt-2 leading-relaxed">
                Kurumsal üye olarak sınırsız ilan yayınlayabilir, XML entegrasyonu kullanabilir ve
                kendi showroom sayfanıza sahip olabilirsiniz.
              </p>
            </div>
          </div>
  ...
  const items = [
    {
      label: "Durum",
      value: trustUI.restrictionState === "active" ? "Aktif" : "Kısıtlı",
      icon: Eye,
      sub:
        trustUI.restrictionState === "active"
          ? "İstatistikler yükleniyor"
          : trustUI.subMessage || "Hesap kısıtlı",
    },
    {
      label: "Güven Durumu",
      value: trustUI.label,
    ...
    {
      label: "Özellikler",
      value: trustUI.isProfessional ? "Kurumsal" : "Bireysel",
      icon: Zap,
      sub: trustUI.isProfessional ? "Sınırsız İlan & XML" : "Sınırlı İlan",
    },
  ];
        ...
        <div className="space-y-4">
          ...
          <div>
            <h2 className="text-2xl font-bold text-foreground tracking-tight">
              {businessName || "Mağazam"}{" "}
              <span className="text-primary/40 font-medium ml-2">Yayında</span>
            </h2>
            <p className="text-sm font-medium text-muted-foreground max-w-lg mt-2">
              Showroom sayfanız şu anda yayında. Müşterileriniz tüm ilanlarınıza tek bir adresten
              ulaşabilir.
            </p>
          </div>
        </div>
        ...
        <div className="flex flex-col sm:flex-row items-center gap-3">
          <div className="flex items-center h-14 w-full sm:w-auto rounded-xl bg-muted/30 border border-border px-5 transition-all hover:bg-muted/50">
            <div className="flex-1 min-w-0 pr-6">
              <span className="text-[9px] font-bold text-muted-foreground/30 uppercase tracking-widest block leading-none mb-1">
                Mağaza URL
              </span>
      ...
      <div className="mt-10 grid grid-cols-1 sm:grid-cols-3 gap-10 pt-10 border-t border-border">
        {items.map((i) => (
          <div key={i.label} className="flex gap-4">
            ...
            <div>
              <div className="text-[10px] font-bold text-muted-foreground/40 uppercase tracking-widest mb-1 leading-none">
                {i.label}
              </div>
              <div className="text-base font-bold text-foreground leading-none tracking-tight">
                {i.value}
              </div>
              <div className="text-xs font-medium text-muted-foreground/50 mt-1.5 leading-none">
                {i.sub}
              </div>
            </div>
          </div>
