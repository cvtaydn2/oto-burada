  ...
  return (
    <div className="grid gap-4 grid-cols-2 lg:grid-cols-4">
      {stats.map((stat) => (
        <div
          key={stat.label}
          className="flex flex-col justify-between rounded-xl border border-border bg-card p-6 transition-all hover:bg-muted/30 shadow-sm group"
        >
          ...
          <div>
            <div className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground/60 mb-1 leading-none">
              {stat.label}
            </div>
            <div className="text-3xl font-bold text-foreground tracking-tighter leading-none">
              {stat.value}
            </div>
          </div>
        </div>
