import { carPartDamageStatuses, carParts, maximumCarYear, minimumListingImages } from "@/lib/constants/domain";
...
export function normalizeDamagePartKey(key: string) {
  const normalized = key.toLocaleLowerCase("tr-TR").replace(/[^a-z0-9]+/g, "_").replace(/^_+|_+$/g, "");
...
export function normalizeDamageStatusValue(value: unknown) {
  ...
  const normalized = value.toLocaleLowerCase("tr-TR").replace(/[^a-z0-9]+/g, "_").replace(/^_+|_+$/g, "");
  const canonical = damageStatusAliases[normalized] ?? normalized;
  return carPartDamageStatuses.includes(canonical as typeof carPartDamageStatuses[number]) ? canonical : null;
}
...
export function normalizeDamageStatusJson(raw?: Record<string, unknown> | null) {
  ...
  return Object.fromEntries(
    Object.entries(raw).map(([k, v]) => {
      ...
      return canonicalKey && canonicalVal && canonicalVal !== "orjinal" ? [canonicalKey, canonicalVal] : null;
...
export function buildDefaultValues(
  ...
  const sortedImages = initialListing ? [...initialListing.images].sort((a, b) => a.order - b.order) : [];
  const rawInspection = initialListing?.expertInspection;
  
  const normalizedInspection = rawInspection && typeof rawInspection === "object"
    ? {
        ...rawInspection,
    ...
    : { hasInspection: false, damageRecord: "bilinmiyor", bodyPaint: "bilinmiyor", engine: "bilinmiyor", transmission: "bilinmiyor", suspension: "bilinmiyor", brakes: "bilinmiyor", electrical: "bilinmiyor", interior: "bilinmiyor", tires: "bilinmiyor", acHeating: "bilinmiyor" };
