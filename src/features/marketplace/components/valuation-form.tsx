      ...
      <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
        ...
        <div className="p-8 rounded-2xl bg-slate-900 text-white shadow-sm space-y-6 relative overflow-hidden">
          ...
          <div className="grid md:grid-cols-3 gap-8 items-center relative z-10">
            ...
            <div className="text-center p-6 rounded-3xl bg-white/5 border border-white/10 shadow-inner">
              <p className="text-sm font-bold text-primary uppercase tracking-[0.2em] mb-2 italic">
                Ortalama
              </p>
              <p className="text-4xl md:text-5xl font-bold text-glow">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4 pt-6 border-t border-white/10">
            <div className="flex items-center gap-2">
              <div
                className={`size-3 rounded-full ${result.confidence === "high" ? "bg-emerald-500" : "bg-amber-500"}`}
              />
              <span className="text-xs font-bold text-slate-300">
                Guven Araligi: {result.confidence === "high" ? "Yuksek" : "Orta"}
              </span>
            </div>
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          ...
          <Button
            ...
            onClick={() =>
              router.push(
                `/listings?brand=${encodeURIComponent(selectedBrand)}&model=${encodeURIComponent(selectedModel)}`
              )
            }
          >
            Piyasayi Gor
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
      <div className="grid md:grid-cols-2 gap-6">
        <div className="space-y-2">
          <Label className="text-xs font-bold uppercase tracking-widest text-muted-foreground italic ml-1">
            Marka
          </Label>
          <Select
            onValueChange={(v) => {
          ...
          {errors.brand && (
            <p className="text-[10px] font-bold text-red-500 ml-1">{errors.brand.message}</p>
          )}
        </div>
        ...
        <div className="space-y-2">
          ...
          <Select disabled={!selectedBrand} onValueChange={(v) => setValue("model", v)}>
            <SelectTrigger className="h-14 rounded-2xl border-border bg-slate-50/50 font-bold">
          ...
          {errors.model && (
            <p className="text-[10px] font-bold text-red-500 ml-1">{errors.model.message}</p>
          )}
        </div>
        ...
        <div className="space-y-2">
          ...
          />
          {errors.year && (
            <p className="text-[10px] font-bold text-red-500 ml-1">{errors.year.message}</p>
          )}
        </div>
        ...
        <div className="space-y-2">
          ...
          />
          {errors.mileage && (
            <p className="text-[10px] font-bold text-red-500 ml-1">{errors.mileage.message}</p>
          )}
        </div>
      </div>
