...
import { carPartDamageStatusLabels } from "@/features/shared/lib/domain";
...
export function VisualDamageMap({ damageStatus, className }: VisualDamageMapProps) {
  ...
  return (
    <div
      className={cn(
        "relative flex items-center justify-center rounded-3xl border border-border/50 bg-muted/30 p-4",
        className
      )}
    >
      <svg viewBox="0 0 240 460" className="h-auto w-full max-w-[180px] drop-shadow-sm">
        ...
        <path
          ...
          className={cn(
            ...
            statusColors[getStatus("sol_on_camurluk")]
          )}
          strokeWidth="2"
        />
        ...
        <path
          ...
          className={cn(
            "transition-colors duration-500",
            statusColors[getStatus("sag_on_camurluk")]
          )}
          strokeWidth="2"
        />
        ...
        <path
          ...
          className={cn(
            "transition-colors duration-500",
            statusColors[getStatus("sol_arka_camurluk")]
          )}
          strokeWidth="2"
        />
        ...
        <path
          ...
          className={cn(
            "transition-colors duration-500",
            statusColors[getStatus("sag_arka_camurluk")]
          )}
          strokeWidth="2"
        />
      ...
      <div className="absolute bottom-2 right-4 flex flex-col gap-1 items-end">
        {(["boyali", "degisen"] as const).map((s) => (
          <div
            key={s}
            className="flex items-center gap-1.5 bg-card/90 backdrop-blur-sm px-2.5 py-1 rounded-lg border border-border shadow-sm"
          >
            <div
              className={cn(
                "size-2.5 rounded-full",
                s === "boyali" ? "bg-amber-400" : "bg-red-400"
              )}
            />
