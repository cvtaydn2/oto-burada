...
export function DashboardListingCard({
  ...
  const isTrustModeCard = trustFilter === "incomplete" && shouldShowTrustReminder;
        ...
        <div className="flex min-w-0 flex-1 flex-col gap-4">
              ...
              <div className="flex flex-wrap gap-2">
                ...
                {shouldShowTrustReminder && (
                  <Link
                    ...
                    className={cn(
                      "w-full rounded-2xl border px-3 py-3 text-left text-blue-900 transition-all sm:w-auto sm:min-w-[320px]",
                      isTrustModeCard
                        ? "border-blue-300 bg-blue-50 px-4 py-4 shadow-sm ring-1 ring-blue-200/70 hover:border-blue-400 hover:bg-blue-100/90 hover:shadow-md"
                        : "border-blue-200 bg-blue-50/85 hover:border-blue-300 hover:bg-blue-100/85"
                    )}
                  >
                    <div className="flex flex-col gap-2.5">
                      <div className="flex items-start justify-between gap-3">
                        <div className="space-y-1">
                          {isTrustModeCard && (
                            <p className="text-[10px] font-bold uppercase tracking-[0.18em] text-blue-700/80">
                              Bu görünümde öncelikli adım
                            </p>
                          )}
                      ...
                      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                        <p className="text-[11px] font-medium leading-4 text-blue-900/75">
                          {isTrustModeCard
                            ? "Trust turundayken bu kartta önce eksik alanları tamamla; diğer işlemler aşağıda ikincil kalır."
                            : "Mevcut düzenleme akışında sadece eksik kalan alanlara odaklan."}
                        </p>
                        <span className="inline-flex shrink-0 items-center justify-center rounded-xl bg-blue-700 px-3 py-2 text-[10px] font-bold uppercase tracking-[0.18em] text-white shadow-sm">
          ...
          <div className="flex flex-col gap-3 border-t border-border/50 pt-3 sm:flex-row sm:items-center sm:justify-between">
            <p className="text-xs text-muted-foreground">
              {isTrustModeCard
                ? "Diğer yönetim aksiyonları hâlâ burada; ancak bu görünümde öncelikli yol trust tamamlama akışıdır."
            ...
            <div
              className={cn(
                ...
                isTrustModeCard && "opacity-80"
              )}
            >
              <Link
                href={`/dashboard/listings?edit=${listing.id}${trustQuery}`}
                className={cn(
                  "inline-flex h-10 items-center justify-center gap-2 rounded-xl border px-3 text-xs font-semibold transition-all",
                  isTrustModeCard
                    ? "border-border/40 bg-muted/25 text-muted-foreground/80 hover:border-border/60 hover:bg-muted/40 hover:text-foreground"
                    : "border-border/60 bg-background text-muted-foreground hover:border-primary/20 hover:bg-primary/5 hover:text-primary"
                )}
              ...
              {isApproved &&
                (!canBump ? (
                  <div
                    className={cn(
                      "inline-flex h-10 items-center justify-center gap-2 rounded-xl border px-3 text-xs font-semibold",
                      isTrustModeCard
                        ? "border-border/30 bg-muted/25 text-muted-foreground/55"
                  ...
                  <Button
                    ...
                    className={cn(
                      "inline-flex h-10 items-center justify-center gap-2 rounded-xl border px-3 text-xs font-semibold transition-all disabled:opacity-30",
                      isTrustModeCard
                        ? "border-border/40 bg-muted/25 text-muted-foreground/80 hover:border-border/60 hover:bg-muted/40 hover:text-foreground"
                        : "border-emerald-100 bg-emerald-50 text-emerald-700 shadow-sm hover:bg-emerald-600 hover:text-white"
                    )}
                    title="Üste Taşı"
                  >
                    {isBumping ? (
              ...
              <Button
                ...
                className={cn(
                  ...
                  isTrustModeCard
                    ? "border-border/40 bg-muted/25 text-muted-foreground/80 hover:border-border/60 hover:bg-muted/40 hover:text-foreground"
                    : isArchived
                      ? "border-slate-800 bg-slate-900 text-white shadow-sm hover:bg-slate-950"
                      : "border-rose-100 bg-rose-50 text-rose-600 shadow-sm hover:bg-rose-600 hover:text-white"
                )}
                  ...
                  <DialogTrigger asChild>
                    <Button
                      ...
                      className={cn(
                        ...
                        isTrustModeCard
                          ? "border-border/40 bg-muted/25 text-muted-foreground/80 hover:border-border/60 hover:bg-muted/40 hover:text-foreground"
                          : "border-amber-100 bg-amber-50 text-amber-700 shadow-sm hover:bg-amber-600 hover:text-white"
                      )}
                      title="Doping Al"
                    >
