...
import { carPartDamageStatusLabels, carPartLabels, carParts } from "@/features/shared/lib/domain";
...
export function VehicleDamageReport({ listing }: { listing: Listing }) {
  ...
  return (
    <section className="rounded-lg border border-border bg-card p-5 shadow-sm mt-6">
      <div className="flex items-center gap-2 mb-5">
        <FileSearch className="text-indigo-600 size-5" />
        <h2 className="text-xl font-bold text-foreground">Tramer ve Kaporta Durumu</h2>
      </div>
      ...
      <div className="grid md:grid-cols-[200px_1fr] gap-6">
        <div className="space-y-4">
          <div className="rounded-xl border border-border bg-muted/30 p-4 h-full">
            ...
            {tramerAmount !== undefined && tramerAmount !== null ? (
              ...
              <p className="text-[13px] font-medium text-muted-foreground flex items-center gap-1.5 border-l-2 border-amber-500 pl-2">
                <ShieldAlert className="size-4 text-amber-500 shrink-0" />
                Belirtilmemiş
              </p>
            )}
            <p className="text-[11px] text-muted-foreground/70 mt-3 leading-tight">
        ...
        <div className="space-y-3">
          <h3 className="text-sm font-bold text-foreground mb-3 border-b border-border/50 pb-2">
            Kaporta Durumu
          </h3>
          {hasStatusFields ? (
              ...
              {activeParts.map((part) => {
                ...
                return (
                  <div key={part} className="flex flex-col gap-1.5">
                    ...
                    <span
                      className={`text-xs font-semibold px-2 py-1 rounded-md border w-fit ${getBadgeColor(status)}`}
                    >
            ...
            <div className="flex flex-col items-center justify-center p-8 text-sm text-muted-foreground bg-muted/30 rounded-lg border border-border border-dashed">
              <span className="font-semibold text-muted-foreground">Detay girilmemiş</span>
              <span className="text-[12px] mt-1 text-center">
                Satıcı Tramer harici kaporta durumu işareti koymamıştır. Lütfen iletişime geçin.
              </span>
            </div>
          )}
        </div>
