...
import { handleAuthRedirects } from "@/features/shared/lib/auth";
import { getSupabaseEnv, hasSupabaseEnv } from "@/features/shared/lib/env";
import {
  applyRequestMetadata,
  applySecurityHeaders,
  generateNonce,
} from "@/features/shared/lib/headers";
...
export async function updateSession(request: NextRequest) {
  ...
  if (!route.isStaticAsset) {
    const { CSRF_COOKIE_HASH_NAME_CLIENT, generateCsrfToken } =
      await import("@/features/shared/lib/csrf");
  ...
  const { getSupabaseProjectRef } = await import("@/features/shared/lib/env");
  ...
  if (
    ...
    const { isMaintenanceGateActive, shouldShowMaintenanceScreen } =
      await import("@/features/shared/lib/maintenance");
  ...
  if (csrfToken) {
    const { applyCsrfCookieToResponse } = await import("@/features/shared/lib/csrf");
