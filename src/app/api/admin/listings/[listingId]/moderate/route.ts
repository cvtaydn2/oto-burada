...
export async function POST(
  ...
  if (!ipRateLimit.allowed) {
    captureServerEvent("admin_listing_moderation_failed", {
      reason: "rate_limited",
      clientIp,
      responseStatus: 429,
    }, "server");
  ...
  try {
    ...
    captureServerError("Admin listing moderation failed", "admin", error, {
      adminUserId: adminUser.id,
      action,
      listingId,
    }, adminUser.id);
    return apiError(API_ERROR_CODES.INTERNAL_ERROR, "Moderasyon işlemi sırasında bir hata oluştu.", 500);
  }
  ...
  captureServerEvent("admin_listing_moderated", {
    adminUserId: adminUser.id,
    action,
    listingId,
