...
export default async function GalleryPage({ params }: GalleryPageProps) {
  ...
  // If failed, try admin/owner bypass
  if (!data && currentUser) {
    data = await getGalleryBySlug(slug, { includeBanned: true, includeUnverified: true });

    // Check if authorized (Admin or Owner)
    if (data && !isAdmin && data.profile.id !== currentUser.id) {
