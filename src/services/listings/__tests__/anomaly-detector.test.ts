...
describe("AI AnomalyDetector (calculateFraudScore)", () => {
  ...
  it("should flag duplicate listing (spam detection)", () => {
    ...
    const result = calculateFraudScore(
      duplicateInput,
      existingListingsBase as unknown as Listing[]
    );
  ...
  it("should flag VIN cloning attempts", () => {
    ...
    const result = calculateFraudScore(
      inputWithVin as ListingCreateInput,
      existingWithVin as unknown as Listing[]
    );

    expect(result.fraudScore).toBeGreaterThanOrEqual(100);
    expect(result.fraudReason).toContain(
      "Aynı şasi numaralı başka bir aktif ilan mevcut (VIN clone)"
    );
  });
