...
import { getOrCreateChat, getChatMessages, sendMessage, getUserChats } from '../chat-service';
...
describe('chat-service', () => {
  ...
  describe('getOrCreateChat', () => {
    ...
    it('should create new chat if not found', async () => {
      ...
      expect(mockChain.insert).toHaveBeenCalledWith({ listing_id: 'l1', buyer_id: 'b1', seller_id: 's1' });
    });
  });
  ...
  describe('sendMessage', () => {
    it('should insert and return new message', async () => {
      ...
      expect(mockChain.insert).toHaveBeenCalledWith({ chat_id: 'c1', sender_id: 'u1', content: 'hello' });
    });
  });
