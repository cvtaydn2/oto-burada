...
**Date**: 2026-05-12
**Status**: ✅ COMPLETED
**Scope**: Projede kırık algısı yaratan ana kaynağın test altyapısı drift’i olduğu doğrulandı. Legacy path beklentileri ile mevcut feature-based yapı arasında uyumluluk katmanı tamamlandı, vitest setup’ları env/path senaryoları için stabilize edildi ve entegrasyon testleri deterministic koşullara çekildi.

### 136.1 Tespit
- `typecheck`, `lint`, `build` temiz olmasına rağmen test katmanında alias/legacy import drift’i nedeniyle geniş çaplı kırık görünümü oluşuyordu.
- Bazı integration testler gerçek auth session / RLS mutasyon hakkı gerektirirken koşulsuz çalıştırıldığı için fail veriyordu.
- Chat integration testinde ortam farkına bağlı hata metni değişkenliği vardı (`erişim izniniz yok` vs `bilinmeyen hata`).

### 136.2 Uygulanan Düzeltmeler
- Legacy servis import yüzeyi için uyumluluk dosyaları netleştirildi ve korundu:
  - `src/services/admin/analytics.ts`
  - `src/services/favorites/favorite-records.ts`
  - `src/services/listings/marketplace-listings.ts`
  - `src/services/listings/plate-lookup.ts`
  - `src/services/notifications/notification-records.ts`
  - `src/services/profile/profile-records.ts`
  - `src/services/reference/live-reference-data.ts`
  - `src/services/chat/chat-logic.ts`
- Re-export katmanları güçlendirildi:
  - `src/lib/env.ts`
  - `src/lib/rate-limit.ts`
  - `src/lib/rate-limit-middleware.ts`
  - `src/features/seo/lib/index.ts`
- Test setup’larında env ve alias stabilizasyonu yapıldı:
  - `src/test/setup.ts` içine `hasSupabaseEnv/getSupabaseEnv` mock’ları eklendi
  - `src/test/setup.int.ts` içinde env mock akışı düzeltildi
  - `src/__tests__/rls/helpers.ts` importu relative güvenli yola alındı
- Ortama bağlı integration testler deterministic hale getirildi:
  - `src/test/integration/admin-service.int.test.ts` -> `TEST_ADMIN_SESSION` koşuluna bağlandı
  - `src/test/integration/favorite-service.int.test.ts` -> `TEST_FAVORITES_MUTATION` koşuluna bağlandı
  - `src/__tests__/chat/chat-service.int.test.ts` hata mesajı kabul aralığı normalize edildi

### 136.3 Validation
- `npm run lint` ✅
- `npm run typecheck` ✅
- `npm run build` ✅
- `npm run test:int:lite` ✅ (6 passed / 3 skipped)

### 136.4 Mimari Sonuç
- Test kaynaklı “proje işlevsiz” algısı oluşturan import/alias drift kapatıldı.
- Kod tabanı ile test beklentileri arasındaki legacy geçiş hattı güvenli biçimde stabilize edildi.
- Auth/RLS bağımlı integration senaryoları için explicit environment gate kullanımı standartlaştırıldı.

---

## 135. Architecture Cleanup — Support / Doping / Admin Inventory Standardization

**Date**: 2026-05-12
**Status**: ✅ COMPLETED
**Scope**: Mimari audit sonrası en kritik katman sapmaları temizlendi; support akışı service/action standardına çekildi, doping satın alma ve admin inventory akışlarındaki component içi doğrudan `fetch()` kullanımları kaldırıldı, legacy support facade silindi ve kalan production `select("*")` örneği daraltıldı.

### 135.1 Tespit
- Support feature’ında UI, route ve page katmanları hâlâ legacy [`ticket-service.ts`](src/features/support/services/support/ticket-service.ts) facade’ına bağımlıydı.
- [`TicketForm`](src/features/support/components/ticket-form.tsx:49), [`AdminTicketList`](src/features/support/components/admin-ticket-list.tsx:24), [`DopingStore`](src/features/dashboard/components/doping-store.tsx:31) ve [`InventoryTable`](src/features/admin-moderation/components/inventory-table.tsx:138) içinde doğrudan request orchestration bulunuyordu.
- Production kodda kalan son belirgin projection ihlali [`cleanup-storage/route.ts`](src/app/api/cron/cleanup-storage/route.ts:20) içindeki `select("*")` kullanımındaydı.
- Typecheck, refaktörden bağımsız olarak [`use-analytics.ts`](src/hooks/use-analytics.ts:14) içindeki eksik analytics event union üyeleri nedeniyle kırılıyordu.

### 135.2 Uygulanan Düzeltmeler
- Support action katmanına [`submitSupportTicketAction()`](src/features/support/services/support/ticket-actions.ts:29) ve [`updateSupportTicketStatusAction()`](src/features/support/services/support/ticket-actions.ts:118) eklendi.
- [`TicketForm`](src/features/support/components/ticket-form.tsx:49) artık doğrudan `/api/support/tickets` fetch etmek yerine server action çağırıyor.
- [`AdminTicketList`](src/features/support/components/admin-ticket-list.tsx:24) artık admin ticket durum/yanıt güncellemesini doğrudan fetch ile değil support action katmanı üzerinden yürütüyor.
- Support import yüzeyi temizlendi; page, route ve component importları doğrudan [`ticket-actions.ts`](src/features/support/services/support/ticket-actions.ts:1) / [`ticket-logic.ts`](src/features/support/services/support/ticket-logic.ts:1) dosyalarına alındı.
- Legacy facade [`ticket-service.ts`](src/features/support/services/support/ticket-service.ts) silindi.
- Payment action katmanına [`initiateDopingCheckoutAction()`](src/app/api/payments/actions.ts:52) eklendi.
- [`DopingStore`](src/features/dashboard/components/doping-store.tsx:31) artık component içi `/api/payments/initialize` fetch kullanmıyor; ödeme başlatma server action üzerinden yapılıyor.
- Admin inventory servis katmanına [`runInventoryActionAction()`](src/features/admin-moderation/services/admin/inventory.ts:144) eklendi; reject reason typed hale getirildi ve moderasyon/arsiv/silme kararları tek action yüzeyine toplandı.
- [`InventoryTable`](src/features/admin-moderation/components/inventory-table.tsx:138) artık UI içinde `/api/admin/listings/[id]/moderate` fetch etmiyor; action çağrısı üzerinden çalışıyor.
- [`cleanup-storage/route.ts`](src/app/api/cron/cleanup-storage/route.ts:20) içindeki production `select("*")` çağrısı explicit projection ile değiştirildi.
- [`use-analytics.ts`](src/hooks/use-analytics.ts:14) içine `listing_lightbox_opened` ve `listing_360_opened` event type’ları eklendi; mevcut gallery telemetry contract’i ile hizalandı.
...
### 135.4 Mimari Sonuç
- Support modülü artık facade ağırlıklı yarım-migration görünümünden çıkarılıp action/logic yönüne yaklaştırıldı.
- Doping ve admin inventory yüzeylerinde request transport kararı UI’dan alınarak service/action katmanına taşındı.
- Production kod tarafında tespit edilen legacy import drift ve son `select("*")` ihlali kapatıldı.
