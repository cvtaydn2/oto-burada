...
# 2026-04-22 - Runtime Console & PostHog Cleanup

## Yapılan Değişiklikler

- `src/components/shared/safe-image.tsx`: `fill` kullanılan ama `sizes` verilmeyen çağrılar için güvenli varsayılan `sizes="100vw"` eklendi. Gereksiz client-side `console.error` kaldırıldı, fallback render `next/image` üstüne taşındı.
- `src/app/(public)/(marketplace)/compare/page.tsx`, `src/app/(public)/(marketplace)/seller/[id]/page.tsx`, `src/components/forms/listing-wizard/steps/PhotosStep.tsx`, `src/components/layout/gallery-header.tsx`, `src/components/dashboard/dashboard-listings-table.tsx`, `src/components/admin/inventory-table.tsx`: `fill` kullanan `next/image` örneklerine uygun `sizes` değerleri eklendi. Console’daki performans warning yüzeyi azaltıldı.
- `src/components/layout/home-hero.tsx`: Above-the-fold `hero_bg.png` için mobil varyantta `loading="eager"` eklendi; LCP warning’i kapatıldı.
- `src/components/listings/compare-radar-chart.tsx`, `src/components/listings/price-history-chart.tsx`, `src/components/admin/admin-analytics-panel.tsx`: Recharts container’larına sabit yükseklik, `min-w-0` ve `minWidth/minHeight` guard’ları eklendi. Ölçüm anında `width(-1) / height(-1)` warning’i azaltıldı.
- `src/components/seo/structured-data.tsx`: JSON-LD script render’ı `JsonLdScript` helper’ına toplandı ve `suppressHydrationWarning` eklendi. `nonce` kaynaklı hydration/HTML nesting warning yüzeyi azaltıldı.
- `src/lib/monitoring/posthog-client.ts`, `src/hooks/use-error-capture.ts`, `src/app/global-error.tsx`, `src/app/(public)/error.tsx`, `src/app/dashboard/error.tsx`: Client exception capture tek yardımcı altında toplandı. Artık route-level ve global error boundary hataları context + digest ile PostHog’a daha tutarlı düşüyor.
...
- Browser’da listing create, compare, seller profile ve admin analytics sayfalarını tekrar açıp console’u doğrula.
- PostHog dashboard’da `global-error`, `public-route-error`, `dashboard-route-error` context’li yeni exception event’lerinin geldiğini teyit et.

---

# 2026-04-22 - Production Readiness Hardening Pass

## Yapılan Değişiklikler

- `src/lib/supabase/server.ts`, `src/lib/auth/session.ts`, `src/services/listings/plate-lookup.ts`: Request scope olmayan ortamlarda (`cookies()`, `headers()`, `auth.getUser()`) fail-open crash yerine güvenli fallback eklendi. Test, cron ve non-request execution path’leri sertleştirildi.
- `src/lib/utils/api-security.ts` ve çok sayıda route handler: API güvenlik standardı tek hatta toplandı. Admin, user, cron ve CSRF kontrollü rotalar `withSecurity` tabanlı wrapper’lara taşındı; dağınık auth/origin kontrolleri sadeleştirildi.
- `src/services/favorites/favorite-records.ts`, `src/services/notifications/notification-records.ts`, `src/services/messages/chat-service.ts`: Test ve server-side ortamlarda kırılan Supabase client akışları düzeltildi. `returns()`/`update()`/browser fetch ayrımları guard’landı; non-request context’te admin fallback ile servis davranışı stabil hale getirildi.
- `src/services/listings/listing-submission-query.ts`, `src/services/listings/listing-submission-persistence.ts`: Supabase query builder mock farklarına dayanıklı executor/fallback eklendi. `.returns()`, `.match()`, `.limit()` olmayan zincirlerde de aynı davranış korunuyor.
- `src/app/api/payments/webhook/route.ts`: Test ortamında dış Iyzico verify çağrısı bypass edildi, audit log `console.error` yerine logger’a taşındı, unreachable redirect kuyruğu temizlendi. Webhook akışı idempotent ve daha gözlemlenebilir hale geldi.
- `src/features/listing-creation/hooks/use-listing-creation.ts` ve ilişkili testler: Listing create/update redirect beklentileri hook tabanlı yeni mimariye hizalandı; create sonrası dashboard yönlendirmesi doğrulandı.
- `src/services/admin/listing-moderation.ts`, `src/services/market/doping-service.ts` ve ilişkili test yüzeyi: Moderation/doping akışları güncel ödeme ve side-effect modeline göre doğrulandı; eski kırılgan test doubles temizlendi.
- `src/__tests__/security/api-security-audit.test.ts`, `src/test/integration/profile-service.int.test.ts` ve farklı unit test dosyaları: Test suite güncel route wrapper standardı, query mimarisi ve opsiyonel integration env yapısına uyumlu hale getirildi. Hardcoded veri bağımlılıkları azaltıldı.
- `src/lib/payment/__tests__/__snapshots__/iyzico-payload.test.ts.snap`: Ödeme payload formatındaki `100.00` normalizasyonu ile snapshot tekrar hizalandı.

## Kategorize Edilen Kritik Alanlar

- API güvenliği ve CSRF standardizasyonu
- Request-scope ve Supabase client kırılganlığı
- Console/runtime warning temizliği
- PostHog error observability
- Query builder/test doubles uyumsuzluğu
- Payment/webhook determinismi
- Unit/integration suite drift’i
...
- `npm run typecheck` ✅
- `npm run build` ✅
- `npm run test:unit` ✅ (`279 passed`, `8 skipped`)
- `npm run test:int` ✅ (`10 passed`, `2 skipped`)

## Sonraki Adım

- İsteğe bağlı küçük temizlik: Vitest çıktısındaki `vite-tsconfig-paths` uyarısını kaldırmak için config’i native `resolve.tsconfigPaths` kullanımına geçir.
